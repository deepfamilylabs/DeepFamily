...
describe("Endorse Tests", function () {
  ...
  async function createBasicPerson() {
    ...
    const personHash = await addPerson(hre.ethers, deepFamily, signers[0], commitment, {
      person,
      tag: "v1",
    });
  ...
  describe("updateEndorsementFee", () => {
    it("allows the owner to update the fee and emits the change", async () => {
      const { deepFamily } = await deployContracts();

      await expect(deepFamily.updateEndorsementFee(750))
        .to.emit(deepFamily, "EndorsementFeeUpdated")
        .withArgs(500, 750);
      expect(await deepFamily.protocolEndorsementFeeBps()).to.equal(750n);
    });

    it("rejects fee updates from non-owners", async () => {
      const { deepFamily, signers } = await deployContracts();

      await expect(deepFamily.connect(signers[1]).updateEndorsementFee(750))
        .to.be.revertedWithCustomError(deepFamily, "OwnableUnauthorizedAccount")
        .withArgs(signers[1].address);
    });

    it("rejects fees above the protocol maximum", async () => {
      const { deepFamily } = await deployContracts();
      const maximum = await deepFamily.PROTOCOL_FEE_BPS_MAX();

      await expect(deepFamily.updateEndorsementFee(maximum + 1n)).to.be.revertedWithCustomError(
        deepFamily,
        "ProtocolFeeTooHigh",
      );
    });
  ...
  it("pays addedBy recipient when different from endorser", async () => {
    ...
    await addPerson(hre.ethers, deepFamily, endorser, fatherCommitment, {
      person: fatherPerson,
      tag: "father",
    });
    await addPerson(hre.ethers, deepFamily, endorser, motherCommitment, {
      person: motherPerson,
      tag: "mother",
    });
    ...
    const targetHash = await addPerson(hre.ethers, deepFamily, addedBy, targetCommitment, {
      person: targetPerson,
      tag: "target",
    });
  ...
  it("routes recipient share to NFT holder when version has been minted", async () => {
    ...
    const personHash = await addPerson(hre.ethers, deepFamily, endorser, cC, {
      ...
      fatherPerson,
      motherPerson,
      fatherIdentityCommitment: fC,
      motherIdentityCommitment: mC,
      fatherVersionIndex: 1,
      motherVersionIndex: 1,
      tag: "child",
    });
  ...
  it("reverts when allowance is insufficient", async () => {
    ...
    await expect(endorseVersion(deepFamily, endorser, personHash, 1)).to.be.revertedWithCustomError(
      token,
  ...
  describe("cancelEndorsement", () => {
    it("reverts when trying to cancel non-existent endorsement", async () => {
      const { deepFamily, reader, personHash } = await createBasicPerson();
      await expect(deepFamily.cancelEndorsement(personHash)).to.be.revertedWithCustomError(
        deepFamily,
        "NotEndorsed",
      );
    });
    ...
    it("successfully cancels endorsement and decrements count", async () => {
      ...
      await expect(deepFamily.connect(endorser).cancelEndorsement(personHash)).to.emit(
        deepFamily,
        "EndorsementCancelled",
      );
