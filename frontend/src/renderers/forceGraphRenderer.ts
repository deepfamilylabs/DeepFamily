...
export function buildForceGraphSimData(graph: TreeGraphData): { nodes: ForceNode[]; links: ForceLink[] } {
  ...
  const links: ForceLink[] = graph.edges.map(e => ({ from: e.from, to: e.to, source: e.from, target: e.to }))
  return { nodes, links }
}
...
export function mountForceGraphScene(params: {
  ...
  const { svgEl, rootEl, graph, nodeUiById, selectedId, actions, drawNodeR, collideNodeR, height, prevNodes, onTick } = params
  ...
  nodeEnter.append('circle').attr('data-ft', 'depth').attr('stroke', 'none').style('pointer-events', 'none')
  ...
  lbl.each(function(d: any) {
    ...
    const width = 60
    gtxt.append('tspan').attr('data-ft', 'shortHash').attr('x', 12).attr('y', -3).attr('font-size', 11)
    gtxt.append('tspan').attr('data-ft', 'version').attr('x', 25 + width).attr('y', -3).attr('text-anchor', 'end').attr('font-size', 11)
    gtxt.append('tspan').attr('data-ft', 'fullName').attr('x', 12).attr('y', 12).attr('font-size', 11)
  })

  nodeEnter.on('click', (_e: any, d: any) => { actions.openNodeById((d as ForceNode).id as any) })
  nodeEnter.on('dblclick', (_e: any, d: any) => { actions.copyHash((d as ForceNode).personHash) })
  ...
  const updateUi = (uiById: Record<string, NodeUi>, selId: string | null) => {
    ...
    nodeGroups.each(function(d: any) {
      ...
      g.select<SVGCircleElement>('circle[data-ft="genderDot"]')
        .attr('fill', getGenderColorHex(ui.gender))
      ...
      badge.select<SVGPathElement>('path[data-ft="endorseStar"]')
        .attr('d', starPath)
      ...
      badge.select<SVGRectElement>('rect[data-ft="endorseHit"]')
        .attr('x', x).attr('y', y).attr('width', badgeW).attr('height', 12)
    })
  }
  ...
  return {
    ...
    getNodePosition: (id: string) => {
      ...
      for (const n of nodes) {
        if (n?.id === id && typeof n.x === 'number' && typeof n.y === 'number') return { x: n.x, y: n.y }
      }
      return null
    }
  }
