...
import { FixedSizeList as VirtualList, type FixedSizeList as VirtualListHandle, type ListChildComponentProps } from 'react-window'
...
export default function TreeListRenderer(props: {
  ...
  const { height, rowHeight, rows, expanded, toggle, selectedKey, nodeUiById, openNodeById, openEndorseById, listRef } = props
  ...
  return (
    <div className="w-full transition-all duration-300 overflow-hidden" style={{ height }}>
      <div className="p-4 pt-16 h-full overflow-x-auto">
        <VirtualList ref={listRef as any} height={height - 32} itemCount={rows.length} itemSize={rowHeight} width={'auto'}>
          {Row}
        </VirtualList>
      </div>
    </div>
