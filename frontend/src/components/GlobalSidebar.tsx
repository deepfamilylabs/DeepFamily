    ...
    <>
      ...
      >
        ...
        <div className="md:hidden p-4 border-b border-gray-100 dark:border-slate-800 flex justify-between items-center bg-white dark:bg-slate-900">
            ...
            <button onClick={closeMobileSidebar} className="p-2 hover:bg-slate-100 dark:hover:bg-slate-800 rounded-full transition-colors">
        ...
        <div className="flex-1 overflow-y-auto overflow-x-hidden custom-scrollbar flex flex-col">
            {menuItems.map((item) => (
                <div key={item.id} className="border-b border-gray-50 dark:border-slate-800/50 md:border-none">
                    ...
                    >
                        <div className={`flex items-center transition-all duration-200 ${(!isMobileOpen && !isExpanded) ? 'justify-center w-full gap-0' : 'gap-3'}`}>
                        ...
                        {(isMobileOpen || isExpanded) && (
                            <div className="flex-shrink-0 ml-2">
                                {item.isSwitch ? (
                                  <div className={`w-10 h-6 rounded-full p-1 transition-colors ${item.value ? 'bg-orange-500' : 'bg-slate-300 dark:bg-slate-600'}`}>
                                      <div className={`w-4 h-4 rounded-full bg-white shadow-sm transition-transform ${item.value ? 'translate-x-4' : ''}`} />
                                  </div>
                                ) : !item.path && (
