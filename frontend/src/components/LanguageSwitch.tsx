...
export default function LanguageSwitch({ variant = "home" }: LanguageSwitchProps) {
  ...
  return (
    <div className="relative" ref={dropdownRef}>
      ...
      <div
        className={`absolute right-0 z-[9999] mt-2 w-40 sm:w-48 origin-top-right rounded-2xl shadow-xl backdrop-blur-xl transition-all duration-200 ${
          isOpen
        ...
        <div className="py-1">
          {languages.map((language, index) => (
            <button
              ...
              className={`flex items-center justify-between w-full px-3 sm:px-4 py-2.5 sm:py-3 text-xs text-left transition-all duration-200 hover:scale-[1.02] ${
                index === 0
                  ? "rounded-t-2xl mt-1"
            ...
            >
              <div className="flex flex-col min-w-0 flex-1">
                <span className="font-semibold text-xs truncate">{language.nativeName}</span>
