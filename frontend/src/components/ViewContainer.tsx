...
export default function ViewContainer({ mode, viewMode, root, subgraphBlock, contractMessage, onViewModeChange, viewModeLabels }: ViewContainerProps) {
  ...
  const content = (
    <Suspense fallback={<LoadingSkeleton />}> {
      isSubgraph ? (
        <div className="w-full bg-gradient-to-br from-white via-slate-50/50 to-blue-50/30 dark:from-slate-900/90 dark:via-slate-800/60 dark:to-slate-900/90 rounded-2xl transition-all duration-300 border border-slate-200/50 dark:border-slate-700/50 shadow-xl backdrop-blur-sm p-4 pt-16 min-h-[747px]">
          {hasRoot ? (
