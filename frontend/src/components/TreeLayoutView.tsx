...
const TreeLayoutView = noPropsForwardRef<FamilyTreeViewHandle>((ref) => {
  ...
  const miniNodes = useMemo(() => positioned.map(pn => ({ id: pn.id, x: pn.x, y: pn.y, w: layout.TREE_NODE_WIDTH, h: layout.TREE_NODE_HEIGHT })), [positioned, layout.TREE_NODE_WIDTH, layout.TREE_NODE_HEIGHT])
    ...
    <>
      ...
      >
        ...
        <g ref={innerRef as any}>
          <TreeLayoutEdges edges={edges} idToPos={idToPos} nodeWidth={layout.TREE_NODE_WIDTH} nodeHeight={layout.TREE_NODE_HEIGHT} />
          <TreeLayoutNodes
            nodes={positioned}
