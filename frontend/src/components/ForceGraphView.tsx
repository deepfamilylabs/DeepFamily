...
import {
  buildForceGraphStructureKey,
  mountForceGraphScene,
  type ForceGraphMiniNode,
...
const ForceGraphView = noPropsForwardRef<FamilyTreeViewHandle>((ref) => {
  ...
  const { transform, zoomIn, zoomOut, setZoom, svgRef, innerRef, kToNorm, normToK, centerOn } =
    useZoom();
  ...
  useEffect(() => {
    actionsRef.current = { openNodeById, openEndorseById, copyHash };
  }, [openNodeById, openEndorseById, copyHash]);
  useEffect(() => {
    nodeUiRef.current = vm.nodeUiById;
  }, [vm.nodeUiById]);
  useEffect(() => {
  ...
  return (
    <GraphViewport
      ...
      miniMapOptions={{
        shape: "circle",
        width: layout.FORCE_MINIMAP_WIDTH,
        height: layout.FORCE_MINIMAP_HEIGHT,
      }}
