...
const HashBadge: React.FC<HashBadgeProps> = ({
  hash,
  onNavigate,
  className = "",
  size = "md",
  ...
  const lp = useLongPress(async () => {
    try {
      await navigator.clipboard.writeText(hash);
      toast.show(defaultCopyMsg);
    } catch {}
  });
  ...
  return (
    ...
    >
      ...
      {showTooltip && showTip && (
        <span
          ref={tipRef}
          className="absolute bottom-full left-1/2 -translate-x-1/2 whitespace-pre rounded bg-black text-white text-sm px-3 py-1 pointer-events-auto z-50 shadow-lg select-text"
          onMouseEnter={show}
