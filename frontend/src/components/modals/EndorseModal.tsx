    ...
    try {
      ...
      const applySuccessFromReceipt = (result: any) => {
        ...
        try {
          ...
          try {
            const endorsedEvents =
              result.logs?.filter((log: any) => {
                try {
                ...
                events: {
                  PersonVersionEndorsed: endorsedEvent
                    ? {
                        ...
                        endorsementFee:
                          endorsedEvent.args?.endorsementFee || deepTokenFeeRaw.toString(),
      ...
      if (result) {
        ...
        setErrorResult({
          ...
          message: t(
            "endorse.transactionFailed",
            "Transaction failed. Please check the error message and try again.",
          ),
          details: t(
            "endorse.transactionFailedDetails",
            "The transaction was submitted but failed to complete successfully.",
          ),
        });
      }
    } catch (error: any) {
