...
export interface UseZoomReturn<
  TSVG extends SVGSVGElement = SVGSVGElement,
  TG extends SVGGElement = SVGGElement,
> {
...
export function useZoom(options: UseZoomOptions = {}): UseZoomReturn {
  ...
  useEffect(() => {
    ...
    const zoom = d3
      ...
      .on("zoom", (event: any) => {
        ...
        if (innerRef.current)
          d3.select(innerRef.current).attr("transform", `translate(${t.x},${t.y}) scale(${t.k})`);
  ...
  const kToNorm = useCallback(
    (k: number) => (Math.log(k) - Math.log(min)) / (Math.log(max) - Math.log(min)),
    [min, max],
  );
  const normToK = useCallback(
    (n: number) => Math.exp(Math.log(min) + n * (Math.log(max) - Math.log(min))),
    [min, max],
  );

  /** Center viewport on graph coordinate (gx,gy) while preserving current scale */
  const centerOn = useCallback(
    (gx: number, gy: number, containerWidth: number, containerHeight: number) => {
      if (!zoomBehaviorRef.current || !svgRef.current) return;
      const k = transform.k;
      ...
      d3.select(svgRef.current)
        .transition()
        .duration(200)
        .call(zoomBehaviorRef.current.transform as any, newT);
    },
    [transform.k],
  );
