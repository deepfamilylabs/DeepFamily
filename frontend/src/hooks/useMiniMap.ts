...
export function useMiniMap(
  { width = 160, height = 120, shape = "rect", nodeSize = 3 }: UseMiniMapOptions,
  { nodes, transform, extent, container, onCenter }: UseMiniMapArgs,
): UseMiniMapReturn {
  ...
  const graphExtentRef = useRef<{ minX: number; maxX: number; minY: number; maxY: number }>({
    minX: 0,
    maxX: 1,
    minY: 0,
    maxY: 1,
  ...
  const updateNodes = useCallback(() => {
    ...
    if (shape === "circle") {
      ...
      sel
        .attr("cx", (d: any) => offsetX + (d.x - minX) * scale)
        .attr("cy", (d: any) => offsetY + (d.y - minY) * scale);
      sel.exit().remove();
    } else {
      const sel = mini
        .select("g.nodes")
      ...
      sel
        ...
        .attr("fill", "#6366f1");
      sel
        .attr("x", (d: any) => offsetX + (d.x - minX) * scale)
        .attr("y", (d: any) => offsetY + (d.y - minY) * scale);
      sel.exit().remove();
    }
    // viewport rectangle
  ...
  const applyPointerToCenter = useCallback(
  ...
  useEffect(() => {
    const mini = d3.select(miniSvgRef.current);
    if (mini.select("g.nodes").empty()) mini.append("g").attr("class", "nodes");
  }, []);
