/**
 * IndexedDB persistence layer for QueryCache.
 *
 * This module provides cold-start acceleration and cross-refresh data retention.
 * It is NOT an independent caching system — the in-memory QueryCache is the
 * authoritative source of truth. IndexedDB only serves as a durable backing store.
 *
 * Falls back to rejecting when IndexedDB is unavailable; callers should handle
 * failures gracefully.
 */
...
async function withStore<T>(
  ...
  cb: (store: IDBObjectStore, resolve: (v: T) => void, reject: (e: any) => void) => void,
): Promise<T> {
