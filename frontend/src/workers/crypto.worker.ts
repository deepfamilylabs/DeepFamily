...
import { decryptMetadataPayload, encryptMetadataJson, passwordFingerprint } from '../lib/metadataCrypto'
import { generateMetadataCID } from '../lib/cid'
import { deriveKeyFromPersonData, type KeyPurpose, type KDFPreset } from '../lib/secureKeyDerivation'
...
type CryptoWorkerMethods = {
  ...
  deriveKey: {
    ...
    result: { key: string; address?: string; timestamp: number; kdfParams: { N: number; r: number; p: number }; purpose: string }
  }
}
...
const getErrorShape = (err: unknown): { message: string; name?: string } => {
  if (err && typeof err === 'object') {
    ...
    if (typeof anyErr.message === 'string') return { message: anyErr.message, name: typeof anyErr.name === 'string' ? anyErr.name : undefined }
  }
  return { message: String(err) }
}

const handlers: {
  [K in keyof CryptoWorkerMethods]: (params: CryptoWorkerMethods[K]['params']) => Promise<CryptoWorkerMethods[K]['result']> | CryptoWorkerMethods[K]['result']
} = {
  ...
  decryptMetadataBundle: async ({ payloadOrJson, password }) => {
    const { plaintext, data, hash, payload } = await decryptMetadataPayload(payloadOrJson, password)
