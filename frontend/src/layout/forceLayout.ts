...
>(params: {
  ...
  const simulation = d3.forceSimulation<TNode>(params.nodes)
    .velocityDecay(cfg.velocityDecay)
    .alpha(cfg.alpha)
    .alphaDecay(cfg.alphaDecay)
    .force('link', d3.forceLink<TNode, any>(params.links).id((d: any) => (d as any).id).distance(cfg.linkDist).strength(cfg.linkStrength))
    .force('charge', d3.forceManyBody().strength(cfg.charge))
    .force('x', d3.forceX<TNode>().x((d: any) => cfg.marginX + (d as any).depth * cfg.gapX).strength(cfg.xStrength))
