...
export function computeTreeLayout(graph: TreeGraphData, rootId: NodeId | null, cfg: TreeLayoutConfig) {
  ...
  function layout(id: NodeId, depth: number): { x: number; y: number } | null {
    ...
    nodes.push({ id, depth, personHash: parsed.personHash, versionIndex: parsed.versionIndex, x, y })
    return { x, y }
  }
