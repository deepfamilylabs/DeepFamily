...
import { nftKey, storyKey, vdKey } from "../../../shared/cache/queryKeys";
import {
 ...
 */
export function createPersonReadGateway(
  contract: any,
  ...
  const listVersionEndorsements = async (
    personHash: string,
    offset: number,
    limit: number,
  ) => {
  ...
  const listTokenUriHistory = async (
    tokenId: string | number,
    offset: number,
    limit: number,
  ) => {
  ...
  const listStoryChunksPage = async (
    tokenId: string | number,
    offset: number,
    limit: number,
  ) => {
