...
const DagView = noPropsForwardRef<FamilyTreeViewHandle>((ref) => {
  ...
  const [hover, setHover] = useState<{ id: string; x: number; y: number; hash: string } | null>(
    null,
  );
  const containerRef = useRef<HTMLDivElement | null>(null);
  const toContainer = (svgX: number, svgY: number) => ({
    left: transform.x + svgX * transform.k,
    top: transform.y + svgY * transform.k,
  });
  ...
  useEffect(() => {
    recomputeHoverPosition();
  }, [transform, recomputeHoverPosition]);
  useEffect(() => {
    if (ctxSelectedId) {
      const p = positions[ctxSelectedId];
      if (p) {
        const w = measuredWidths[ctxSelectedId] || nodeWidth;
        const scr = toContainer(p.x + w / 2, p.y);
        setHover((prev) => {
          if (prev && prev.id === ctxSelectedId && prev.x === scr.left && prev.y === scr.top)
            return prev;
  ...
  const miniNodes = useMemo(
    () =>
      nodes.map((n) => ({
        ...
        x: positions[n.id].x,
        y: positions[n.id].y,
        w: measuredWidths[n.id] || nodeWidth,
        h: nodeHeight,
      })),
    [nodes, positions, measuredWidths, nodeWidth, nodeHeight],
  );
  ...
  return (
    <>
      <GraphViewport
        ...
        height="100%"
        containerClassName="relative w-full h-full overflow-hidden bg-linear-to-br from-white via-slate-50/50 to-orange-50/30 dark:from-slate-900/90 dark:via-slate-800/60 dark:to-slate-900/90 transition-all duration-300 overscroll-contain"
