...
export default function EndorseModal({
  ...
  const [userDeepBalanceRaw, setUserDeepBalanceRaw] = useState<bigint>(0n);
  ...
  useEffect(() => {
    ...
    const changed =
      prevTargetRef.current.hash !== nextHash || prevTargetRef.current.index !== nextIndex;
    if (!changed) return;
  ...
  useEffect(() => {
    const loadEndorsementData = async () => {
      ...
      try {
        ...
        const tokenContract = createDeepTokenContract(deepTokenContract, contract.runner);
        ...
        setUserDeepBalanceRaw(BigInt(balance));
            ...
            if (details) {
              ...
              if (tokenId > 0 && getNFTDetails) {
                try {
                  ...
                  name =
                    (nftDetails as any)?.coreInfo?.supplementInfo?.fullName ||
              ...
              if (!name) {
                name =
                  (details as any)?.version?.coreInfo?.supplementInfo?.fullName ||
              ...
              if (tokenId > 0) {
                const nftHolder =
                  (await getOwnerOf(String(tokenId))) || (await contract.ownerOf(tokenId));
  ...
  // React to endorseFlow state changes
  useEffect(() => {
    const s = endorseFlow.status;
    setIsSubmitting(s === "validating" || s === "approving" || s === "submitting" || s === "confirming");
    setIsApproving(s === "approving");
  }, [endorseFlow.status]);
  ...
  useEffect(() => {
    ...
    setErrorResult(null);
    setDeepTokenFee(result.feeFormatted);
    setDeepTokenFeeRaw(result.fee);
    setDeepTokenDecimals(result.decimals);
    setDeepTokenSymbol(result.symbol);
    ...
    if (nextBalance >= 0n) {
      setUserDeepBalance(ethers.formatUnits(nextBalance, result.decimals));
      setUserDeepBalanceRaw(nextBalance);
    }
      ...
      events: {
        PersonVersionEndorsed: result.event
          ? {
              ...
              timestamp: result.event.timestamp || Math.floor(Date.now() / 1000),
            }
          : {
              personHash: targetPersonHash!,
              endorser: address,
    ...
    onSuccess?.({ ...result.receipt, endorsementCount: currentEndorsementCount + 1 });
  }, [endorseFlow.status, endorseFlow.result]);

  useEffect(() => {
    if (endorseFlow.status !== "error" || !endorseFlow.error) return;
    const error = endorseFlow.error as any;
  ...
  }, [endorseFlow.status, endorseFlow.error]);
  ...
  const handleEndorse = async (isRetry = false) => {
    ...
    if (userDeepBalanceRaw < deepTokenFeeRaw) {
    ...
    endorseFlow.run({
      personHash: targetPersonHash!,
      versionIndex: Number(targetVersionIndex),
      deepTokenAddress: deepTokenAddress || undefined,
      suppressToasts: false,
      onStageChange: (stage) => {
        setIsSubmitting(stage === "checking" || stage === "approving" || stage === "submitting");
                        ...
                        {isNFTMinted ? (
                          <>
                            ...
                            {t(
                              ...
                              {
                                recipientPercent: (10000 - protocolFeeBps) / 100,
                                protocolPercent: protocolFeeBps / 100,
                              },
                            )}
                          </>
                        ) : (
                    ...
                    <div className="min-w-0 flex-1">
                      ...
                      <p className="text-xs font-medium text-orange-700 dark:text-orange-300">
                        {isApproving
                          ...
                          : t(
                              "endorse.endorsingDesc",
                              "Processing endorsement on the blockchain...",
                            )}
                      </p>
                    </div>
                  </div>
                        ...
                        <div className="pt-2 border-t border-green-200/50 dark:border-green-700/50">
                          ...
                          <div className="space-y-2">
                            ...
                            <DataRow
                              ...
                              value={new Date(
                                Number(successResult.events.PersonVersionEndorsed.timestamp) * 1000,
                              ).toLocaleString()}
                              colorClass="green"
                              isPlainText
                            />
