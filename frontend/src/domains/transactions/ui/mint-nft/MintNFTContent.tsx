...
export default function MintNFTModal({
  ...
  const mintNftFlow = useMintNftFlow();
  ...
  const onSubmit = async (data: any) => {
    ...
    try {
      ...
      setSuccessResult({
        ...
        events: {
          PersonNFTMinted: mintResult.event
            ? {
                personHash: mintResult.event.personHash,
                tokenId: mintResult.event.tokenId || tokenId,
                owner: mintResult.event.owner || address,
                versionIndex: mintResult.event.versionIndex || finalVersionIndex,
                tokenURI: mintResult.event.tokenURI || processedData.tokenURI || "",
                timestamp: mintResult.event.timestamp || Math.floor(Date.now() / 1000),
              }
            : null,
        },
      });
          ...
          <div className="flex-1 overflow-y-auto overscroll-contain overflow-x-hidden min-h-0 touch-pan-y">
            ...
            >
              <div className="flex-1 p-6 space-y-6">
                ...
                {successResult && (
                  <MintNFTSuccessResult t={t} successResult={successResult} />
                )}
