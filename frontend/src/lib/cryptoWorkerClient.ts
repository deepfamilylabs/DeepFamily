...
type CryptoWorkerCallMap = {
  ...
  deriveKey: { params: { input: HashForm; purpose?: KeyPurpose; preset?: KDFPreset }; result: DerivedKey }
}
...
const ensureWorker = (): Worker => {
  ...
  workerSingleton = new Worker(new URL('../workers/crypto.worker.ts', import.meta.url), { type: 'module' })
  workerSingleton.addEventListener('message', (event: MessageEvent<CryptoWorkerResponse>) => {
    ...
    else entry.reject(Object.assign(new Error(msg.error?.message || 'Crypto worker error'), { name: msg.error?.name }))
  })
