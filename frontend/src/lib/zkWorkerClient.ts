...
type ZkWorkerCallMap = {
  generatePersonProof: {
    params: { person: PersonData; father: PersonData | null; mother: PersonData | null; submitterAddress: string }
    result: { proof: Groth16Proof; publicSignals: string[] }
  }
  verifyPersonProof: { params: { proof: Groth16Proof; publicSignals: string[] }; result: { ok: boolean } }
  generateNamePoseidonProof: {
    params: { fullName: string; passphrase: string; minterAddress: string }
    result: { proof: Groth16Proof; publicSignals: string[] }
  }
  verifyNamePoseidonProof: { params: { proof: Groth16Proof; publicSignals: string[] }; result: { ok: boolean } }
}
...
const ensureWorker = (): Worker => {
  ...
  workerSingleton = new Worker(new URL('../workers/zk.worker.ts', import.meta.url), { type: 'module' })
  workerSingleton.addEventListener('message', (event: MessageEvent<ZkWorkerResponse>) => {
    ...
    else entry.reject(Object.assign(new Error(msg.error?.message || 'ZK worker error'), { name: msg.error?.name }))
  })
