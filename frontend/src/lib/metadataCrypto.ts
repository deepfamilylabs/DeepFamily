...
const safeFromBase64 = (b64: string, opts: { maxBytes: number; label: string }): Uint8Array<ArrayBuffer> => {
...
export const decryptMetadataPayload = async (
  ...
  const salt = safeFromBase64(payload?.kdf?.salt || '', { maxBytes: MAX_SALT_BYTES, label: 'kdf.salt' })
  ...
  const cipherBytes = safeFromBase64(payload?.ciphertext || '', { maxBytes: MAX_CIPHERTEXT_BYTES, label: 'ciphertext' })
