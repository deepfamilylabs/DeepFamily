...
export default function DecryptMetadataPage() {
  ...
  const handleDecrypt = async () => {
    ...
    if (!encryptedJson.trim()) {
      setError(t('decryptMetadata.payloadRequired', 'Please fetch or paste encrypted content first'))
      return
    }
    try {
      ...
      const localizedMessage =
        message === 'Web Crypto is not available in this environment'
          ? t('decryptMetadata.cryptoUnavailable', 'Web Crypto is not available in this environment')
          : message === 'Plaintext hash verification failed'
            ? t('decryptMetadata.hashMismatch', 'Plaintext hash verification failed, password or file may be incorrect')
            : message
      setError(localizedMessage || t('decryptMetadata.decryptFailed', 'Decryption failed, please check password or file'))
      setResult(null)
    } finally {
  ...
  return (
    <div className="space-y-6 text-gray-900 dark:text-gray-100 pb-4 md:pb-0">
      <div className="flex flex-wrap items-center gap-3 justify-between">
        <h1 className="text-2xl sm:text-3xl font-semibold">{t('decryptMetadata.title', 'Decrypt Metadata')}</h1>
      ...
      <div className="grid lg:grid-cols-[1fr_0.9fr] gap-4 lg:gap-6">
        <div className="space-y-4">
            ...
            <div className="grid sm:grid-cols-2 gap-3">
              <div className="space-y-1.5">
                <label className="text-xs font-medium text-gray-700 dark:text-gray-200">
                  {t('decryptMetadata.baseUrl', 'Base URL (combined with CID to form download address)')}
                </label>
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-medium text-gray-700 dark:text-gray-200 flex items-center gap-1.5">
                ...
                <button
                  ...
                  aria-label={
                    showPassword ? t('decryptMetadata.hidePassword', 'Hide password') : t('decryptMetadata.showPassword', 'Show password')
                  }
                >
            ...
            {payloadMeta && (
              <div className="grid sm:grid-cols-3 gap-2 text-xs">
                <div className="flex flex-col gap-1 px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-950">
                  ...
                  <span className="text-gray-800 dark:text-gray-100 font-medium">{payloadMeta.version || '—'}</span>
                </div>
                <div className="flex flex-col gap-1 px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-950">
                  <span className="text-[11px] uppercase tracking-wide text-gray-500 dark:text-gray-400 font-semibold">Schema</span>
                  <span className="text-gray-800 dark:text-gray-100 font-medium break-words">{payloadMeta.schema || payloadMeta.aad || '—'}</span>
                </div>
                <div className="flex flex-col gap-1 px-3 py-2 rounded-lg border border-gray-200 dark:border-gray-800 bg-gray-50 dark:bg-gray-950">
                  <span className="text-[11px] uppercase tracking-wide text-gray-500 dark:text-gray-400 font-semibold">
                    {t('decryptMetadata.payloadMeta.cipher', 'Algorithm')}
                  </span>
                  <span className="text-gray-800 dark:text-gray-100 font-medium">{payloadMeta.cipher || '—'}</span>
                </div>
              </div>
            )}
          </div>
          ...
          <div className="flex justify-end">
            ...
            >
              {isDecrypting ? <Loader2 className="w-4 h-4 animate-spin" /> : <Eye className="w-4 h-4" />}
        ...
        <div className="space-y-3">
          <div className="rounded-xl border border-gray-200 dark:border-gray-800 bg-white dark:bg-gray-900 shadow-sm p-5 min-h-[260px]">
            <div className="flex items-center gap-2 mb-3 text-sm font-semibold">
              ...
              <span className="text-gray-900 dark:text-gray-100">{t('decryptMetadata.result', 'Decryption Result')}</span>
            </div>
            {result ? (
              ...
              <p className="text-xs text-gray-600 dark:text-gray-400">
                {t('decryptMetadata.resultPlaceholder', 'Decrypted plaintext metadata will be displayed here')}
              </p>
            )}
          </div>
        </div>
