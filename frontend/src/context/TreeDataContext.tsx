...
export function TreeDataProvider({ children }: { children: React.ReactNode }) {
  ...
  const contract = useMemo(() => {
    ...
    try {
      return new ethers.Contract(contractAddress, (DeepFamily as any).abi, provider);
    } catch {
      return null;
    }
  ...
  const api = useMemo(
    () => (contract ? createDeepFamilyApi(contract, queryCacheRef.current) : null),
    [contract],
  );
  ...
  useEffect(() => {
    ...
    const loadChildrenUnion = async (
      ...
      const { childIds, totalVersions } = await api.listChildrenUnionAll(personHash, {
        ...
        totalVersionsOptions: {
          ...
          onFetched: () => {
            debugStatsRef.current.lastTotalVersionsFetchAt = Date.now();
          },
        },
        onTotalVersions: (tv) => {
    ...
    const scheduleRevalidate = (key: string, run: () => Promise<void>) => {
      ...
      run()
        .catch(() => {})
        .finally(() => {
          edgeRevalidateRef.current.delete(key);
        });
    };

    const ensureReachableChildren = (
      ...
      if (parentId) {
        ...
        for (const rid of reachableIds) {
          if (parseNodeId(rid).personHash.toLowerCase() === key) {
    ...
    (async () => {
      ...
      try {
        ...
        if (!cancelled) {
          ...
          const code = (e?.code ??
            ...
            (e?.info && e.info.error && e.info.error.code) ??
            (e?.cause && e.cause.code)) as any;
          const isRateLimit =
            code === -32005 ||
            /Too\s*many\s*requests|daily\s*request\s*count\s*exceeded|rate[-\s]?limit|status\s*429/i.test(
              raw,
            );
          const isConnRefused = /ECONNREFUSED|ERR_CONNECTION_REFUSED|connection\s*refused/i.test(
            raw,
          );
          const isAbort = /AbortError|The user aborted a request/i.test(raw);
          const isFetchFail =
            /Failed\s*to\s*fetch|NetworkError\s*when\s*attempting\s*to\s*fetch/i.test(raw);
          const isNetwork =
            isConnRefused ||
            isAbort ||
            isFetchFail ||
            /network|timeout|ECONN|ENET|EAI_AGAIN/i.test(raw) ||
      ...
      try {
        ...
        if (!cancelled) {
          ...
          const isFetchFail =
            /Failed\s*to\s*fetch|NetworkError\s*when\s*attempting\s*to\s*fetch/i.test(msg);
          const isNetwork =
            isConnRefused ||
            isAbort ||
            isFetchFail ||
            /network|timeout|ECONN|ENET|EAI_AGAIN/i.test(msg) ||
            String(code).includes("NETWORK");
          const isRootInvalid =
            name.includes("InvalidPersonHash") ||
            name.includes("InvalidVersionIndex") ||
            /InvalidPersonHash|InvalidVersionIndex/i.test(msg);
          if (isRateLimit) setContractMessage(t("familyTree.status.rateLimited"));
          else if (isRootInvalid) setContractMessage(t("familyTree.status.rootNotFound"));
          else if (isNetwork) setContractMessage(t("familyTree.status.networkError"));
          else setContractMessage(t("familyTree.status.contractModeRootNotFound"));
          if (isRootInvalid) setRootExists(false);
          if (!stageLoggedRef.current.has("root_check")) {
            stageLoggedRef.current.add("root_check");
            push(e as any, { stage: "root_check" });
          }
        }
        setLoading(false);
        return;
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const hasRequiredFields = (nd?: NodeData) =>
        !!nd && nd.endorsementCount !== undefined && nd.tokenId !== undefined;
      ...
      for (let i = 0; i < nodePairs.length && !cancelled; i += 40) {
        ...
        try {
          ...
          results.forEach((entry) => {
            ...
            const { parsed, original } = entry as {
              ok: true;
              parsed: ReturnType<typeof parseVersionDetailsResult>;
              original: { h: string; v: number };
            };
            const id = makeNodeId(original.h, original.v);
            const versionDetailsKey = vdKey(original.h, original.v);
            const vdFetchedAt =
              queryCacheRef.current.getEntry(versionDetailsKey)?.fetchedAt ?? Date.now();
          ...
          for (const item of pendingNFTs) {
            ...
            try {
              ...
              if (!stageLoggedRef.current.has("nft_details_batch")) {
                stageLoggedRef.current.add("nft_details_batch");
                push(e as any, { stage: "nft_details_batch" });
              }
            }
          }
        } catch (e) {
          if (!stageLoggedRef.current.has("counts_batch")) {
            ...
            push(e as any, { stage: "counts_batch" });
          }
        }
      }
      if (!cancelled) setEndorsementsReady(true);
  ...
  const getNodeByTokenId = useCallback(
    async (tokenId: string): Promise<NodeData | null> => {
  ...
  const getStoryData = useCallback(
    async (tokenId: string, opts?: { nodeIdHint?: string }) => {
      const scheduleStoryRevalidate = (key: string, run: () => Promise<void>) => {
        ...
        run()
          .catch(() => {})
          .finally(() => {
            storyRevalidateRef.current.delete(key);
          });
      };
      ...
      const fetchAndStoreStory = async (effectiveTokenId: string, nodeIdToUpdate?: string) => {
        ...
        if (total > 0) {
          ...
          if (offset < total) {
            ...
            while (hasMore && offset < total) {
              const out: any = await contract!.listStoryChunks(
                effectiveTokenId,
                offset,
                STORY_PAGE_LIMIT,
              );
              ...
              for (const raw of rawChunks) {
                ...
                if (Number.isFinite(c.chunkIndex) && c.chunkIndex >= 0)
                  existingMap.set(c.chunkIndex, c);
              }
        ...
        const lengthMatch = storyMetadata?.totalLength
          ? computedLength === storyMetadata.totalLength
          : true;
        let hashMatch: boolean | null = null;
        let computedHash: string | undefined;
        if (
          missing.length === 0 &&
          storyMetadata?.totalChunks > 0 &&
          storyMetadata?.fullStoryHash &&
          storyMetadata.fullStoryHash !== ethers.ZeroHash
        ) {
      ...
      if (!nodeId) {
        try {
          const nd = await getNodeByTokenId(tokenId);
          if (nd) {
            nodeId = nd.id;
            ndFromLookup = nd;
          }
      ...
      if (nodeId) {
        ...
        if (nd?.storyMetadata && Array.isArray(nd?.storyChunks)) {
          ...
          const computedLength = sorted.reduce(
            (acc, c) => acc + encoder.encode(c.content).length,
            0,
          );
          const missing: number[] = [];
          ...
          if (
            ...
            nd.storyMetadata?.fullStoryHash &&
            nd.storyMetadata.fullStoryHash !== ethers.ZeroHash
          ) {
            computedHash = computeStoryHash(sorted);
          ...
          return {
            ...
            integrity: {
              missing,
              lengthMatch: nd.storyMetadata
                ? computedLength === nd.storyMetadata.totalLength
                : true,
              hashMatch,
              computedLength,
              computedHash,
  ...
  const refreshInvalidatedEdges = useCallback(
    async (invalidation: ReturnType<typeof getInvalidateKeysAfterPersonVersionAdded>) => {
      ...
      const reachableHashes = new Set(
        Array.from(reachableIds).map((id) => parseNodeId(id).personHash.toLowerCase()),
      );
      ...
      for (const parentHashLower of invalidation.unionKeys) {
        ...
        try {
          const { childIds, totalVersions } = await api.listChildrenUnionAll(parentHashLower, {
            ...
            onTotalVersions: (tv) => {
              if (tv > 0) updateTotalVersions(parentHashLower, tv);
            },
          });
  ...
  const parseTxEvents = useCallback(
    (receipt: any) => {
      const parsed = {
  ...
  const invalidateByTx = useCallback(
    (
      ...
      const parsed = input.receipt
        ? parseTxEvents(input.receipt)
        : {
            PersonVersionAdded: [] as Array<{
              personHash: string;
              versionIndex: number;
      ...
      if (input.events?.PersonVersionAdded)
        parsed.PersonVersionAdded.push(input.events.PersonVersionAdded);
      if (input.events?.PersonVersionEndorsed)
        parsed.PersonVersionEndorsed.push(input.events.PersonVersionEndorsed);
      ...
      for (const ev of parsed.PersonNFTMinted) {
        ...
        if (
          ev.personHash &&
          typeof ev.versionIndex === "number" &&
          Number.isFinite(ev.versionIndex) &&
          ev.versionIndex > 0
        ) {
          versionDetailKeys.add(vdKey(ev.personHash, ev.versionIndex));
        }
  ...
  const preloadStoryData = useCallback(
    (tokenId: string) => {
