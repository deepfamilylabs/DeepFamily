...
export function assertDisclosureBindingPublicSignalsMatch(built, publicSignals) {
  ...
  for (const fieldName of DISCLOSURE_BINDING_V2_PUBLIC_SIGNAL_SPEC.fieldOrder) {
...
export async function generateDisclosureBindingProof(person, minterAddress, opts = {}) {
  ...
  // snarkjs 0.7.x keeps its ffjavascript curve worker pool alive after fullProve(). These helpers
  // are also used by one-shot Hardhat tasks, so use its supported single-thread prover mode to
  // avoid retaining worker MessagePorts after a proof has been returned.
