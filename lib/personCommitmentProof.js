...
const DEFAULT_WASM_CANDIDATES = [
  path.join(__dirname, "../frontend/public/zk/person_commitment.wasm"),
  path.join(__dirname, "../zk-artifacts/circuits/person_commitment_js/person_commitment.wasm"),
  path.join(__dirname, "../zk-artifacts/circuits/person_commitment.wasm"),
  path.join(__dirname, "../artifacts/circuits/person_commitment_js/person_commitment.wasm"),
  path.join(__dirname, "../artifacts/circuits/person_commitment.wasm"),
];

const DEFAULT_ZKEY_CANDIDATES = [
  path.join(__dirname, "../frontend/public/zk/person_commitment_final.zkey"),
  path.join(__dirname, "../zk-artifacts/circuits/person_commitment_final.zkey"),
  path.join(__dirname, "../artifacts/circuits/person_commitment_final.zkey"),
];
...
function normalizeSignals(publicSignals) {
  if (!Array.isArray(publicSignals) || publicSignals.length !== 7) {
    throw new Error(`Person commitment public signals length mismatch (expected 7, got ${publicSignals?.length})`);
  }
...
export async function generatePersonCommitmentProof(
  ...
  return {
    ...
    publicSignalsStruct: {
      ...
      fatherIdentityCommitment: normalizedSignals[1],
      motherIdentityCommitment: normalizedSignals[2],
      submitter: normalizedSignals[3],
      schemaVersion: normalizedSignals[4],
      cryptoSuiteVersion: normalizedSignals[5],
      hashAlgoId: normalizedSignals[6],
    },
