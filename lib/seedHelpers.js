...
import { generatePersonCommitmentProof } from "./personCommitmentProof.js";
import { generateDisclosureBindingProof } from "./disclosureBindingProof.js";
import { DEFAULT_PROOF_SYSTEM_ID } from "./proofCommon.js";
...
function packBirthGenderField({ birthYear, birthMonth, birthDay, gender, isBirthBC }) {
  return (
    (BigInt(birthYear || 0) << 25n) |
    (BigInt(birthMonth || 0) << 17n) |
    (BigInt(birthDay || 0) << 9n) |
    (BigInt(gender || 0) << 1n) |
    (isBirthBC ? 1n : 0n)
  );
}
...
function computeVersionSubjectHash(personHash, versionIndex) {
...
function computeTokenSubjectHash(tokenId) {
  return ethers.keccak256(
    ethers.AbiCoder.defaultAbiCoder().encode(
      ["string", "uint256"],
...
function computeCoreInfoDigest(coreInfo) {
  return ethers.keccak256(
    ethers.AbiCoder.defaultAbiCoder().encode(
      [
        "bytes32",
...
async function getAttestationDomain(deepFamily, signer) {
  const provider = signer.provider || deepFamily.runner?.provider;
  if (!provider) throw new Error("Provider is required to build attestation ref");
  const network = await provider.getNetwork();
  return {
    chainId: network.chainId,
    contractAddress: await deepFamily.getAddress(),
  };
}

async function makeBaseAttestationRef({
  deepFamily,
  signer,
  subjectType,
  subjectHash,
  ...
  const payloadDigest = ethers.keccak256(
    ethers.AbiCoder.defaultAbiCoder().encode(
      ...
      [actionDigest, signerAddress, nonce],
    ),
  );
  ...
  return {
    ...
    signerKeyId: ethers.zeroPadValue(signerAddress, 32),
    uri: `ipfs://seed-attestation-${nonce.toString()}`,
...
async function makeEndorseAttestationRef({ deepFamily, signer, personHash, versionIndex }) {
  const { chainId, contractAddress } = await getAttestationDomain(deepFamily, signer);
  const actor = await signer.getAddress();
  const subjectHash = computeVersionSubjectHash(personHash, versionIndex);
...
async function makeMintAttestationRef({
  deepFamily,
  signer,
  personHash,
  versionIndex,
  ...
  const actionDigest = ethers.keccak256(
    ethers.AbiCoder.defaultAbiCoder().encode(
      ...
      [
        ...
        computeCoreInfoDigest(coreInfo),
      ],
    ),
  );
  return makeBaseAttestationRef({
...
async function makeSealStoryAttestationRef({ deepFamily, signer, tokenId }) {
  const { chainId, contractAddress } = await getAttestationDomain(deepFamily, signer);
  const actor = await signer.getAddress();
  const metadata = await deepFamily.storyMetadata(tokenId);
  const subjectHash = computeTokenSubjectHash(tokenId);
  const actionDigest = ethers.keccak256(
    ethers.AbiCoder.defaultAbiCoder().encode(
...
async function addPersonVersion({
  ...
  const generatedProof = await generatePersonCommitmentProof(
    normalizedPersonData,
    fatherData ? normalizePersonData(fatherData) : null,
    motherData ? normalizePersonData(motherData) : null,
    submitter,
    {
      proofSystemId,
      schemaVersion,
  ...
  const proof = generatedProof.proofEnvelope;
  const publicSignals = generatedProof.publicSignalsStruct;
  ...
  return {
    ...
    tx,
    receipt,
    timing: { proofGeneration: proofDuration, transaction: txDuration, total: proofDuration + txDuration },
  };
}
...
async function endorseVersion({
  ...
  const tx = await deepFamily.connect(signer).endorseVersion(
    personHash,
    versionIndex,
    await makeEndorseAttestationRef({ deepFamily, signer, personHash, versionIndex }),
  );
  const receipt = await tx.wait();
...
async function mintPersonVersionNFT({
  ...
  const generatedProof = await generateDisclosureBindingProof(normalizedBasic, signerAddr, {
    proofSystemId,
    schemaVersion,
    cryptoSuiteVersion,
    hashAlgoId,
