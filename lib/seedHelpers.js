...
async function mintPersonVersionNFT({
  ...
  const storyTitle = normalizedSupplement.storyTitle ?? "";
  ...
  if (story === "" && storyTitle.trim()) {
    throw new Error("Mint biography title requires biography content");
  }
  ...
  try {
    ...
    if (story !== "") {
      const decoded = decodeStoryRecord(storyPayload);
      if (decoded.title !== storyTitle || decoded.content !== story) {
    ...
    if (story !== "") {
      ...
      if (
        restored.decoded?.title !== storyTitle ||
        restored.decoded?.content !== story ||
        restored.decoded?.recordType !== 0
      ) {
